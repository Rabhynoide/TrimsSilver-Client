...
- The `.github/workflows/publish.yml` "Build and Release" workflow triggers automatically on push of a `v.*` tag (e.g. `v.0.40.0.0`, or `v.0.40.0.0-beta` for a prerelease) — pushing such a tag builds Windows/Linux/macOS, creates the GitHub Release, and uploads installers, with no manual step. `workflow_dispatch` still works too (e.g. to retry a failed run against an existing tag).
- Never publish, create, edit, or delete a GitHub release or release tag unless the user explicitly instructs you to publish that specific release — this now also means never pushing a `v.*` tag, since that alone triggers the full release. Preparing release-related code, versions, or workflow changes does not authorize pushing a release tag or running the release workflow.
- Publish stable releases only from `master` and beta prereleases only from `beta`. The tag must point at a commit reachable from the matching branch (`origin/master` for a plain `v.<version>` tag, `origin/beta` for a `v.<version>-beta` tag) — the `validate` job enforces this and fails the run otherwise.
